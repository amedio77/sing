...
> **v0.4 변경**: ③ **모드 D — 피아노 코드(화음)** 추가. 삼화음(메이저·마이너·증·감) + 7화음(dom7·maj7·m7)을 **양방향**(코드↔구성음)으로 학습. `playChord()`로 화음 재생. `data/chords.js` 신설(§6.5).
> **v0.5 변경**: ④ **악보(오선) 표현 확장** — 모드 B/C/D도 텍스트/칩 대신 실제 오선으로 표현. 임시표(♯/♭)·화음(쌓인 음표)·조표(표준 위치) 렌더를 `staff.js`에 추가(§5.9).
> **v0.6 변경**: ⑤ **문제 풀 확장(다양성↑)** — 모드 A는 덧줄 음을 포함해 난이도별 음역 확대(쉬움=오선 안 dvRef..+8, 보통=덧줄 포함 dvRef−3..+10), 모드 B는 옥타브 랜덤(4·5)으로 오선 위치 다양화, 모드 C는 쉬움에도 샤프/플랫 혼합. 매 문제 랜덤은 기존과 동일하되 풀이 커져 반복 체감 감소.
...
│   └── chords.js    # 코드(화음) 품질 상수 + makeChord 팩토리 (스펠링/MIDI 계산)
│
└── src/
...
### 5.9 악보 표현 확장 (모드 B·C·D) — v0.5

모드 A만 오선을 쓰던 것을 B·C·D로 확장한다. `staff.js`에 세 렌더러를 추가한다.

**임시표(accidental) 렌더**: 유니코드 ♯(U+266F)·♭(U+266D)·♮(U+266E)는 **일반 텍스트 폰트에서 안정적**(음자리표 글리프와 달리). `<text>`로 음표 머리 **왼쪽(≈ NOTE_X−16)**, 해당 음의 y에 배치. `drawNote(note, clef)`가 `note.acc`(또는 `accidental`)이 있으면 임시표를 함께 그린다.

**화음 렌더 `drawChord(tones, clef)`**: 같은 x(NOTE_X)에 각 음표 머리를 `noteToY`로 쌓고, 임시표는 각 머리 왼쪽에. 화음 구성음은 3도 간격이라 머리끼리 세로로 인접(≈12px)해 충돌 없음. 오선 밖 음은 기존 `ledgerYs`로 덧줄. 화음은 표준 관례상 **높은음자리표**로 표시.

**조표 렌더 `drawKeySignature(sign, letters, clef=treble)`**: 조표는 오선의 **표준 위치**에 순서대로(x 증가) 놓인다. 높은음자리표 표준 위치(SHARP_ORDER / FLAT_ORDER 순서와 동일 인덱스):
...
> 위 (음이름, 옥타브)의 y는 `noteToY`로 계산(하드코딩 아님). 예) F5→y40(top line), C5→y58(3번째 칸), B♭ 위치 B4→y64(가운데 줄).

**모드별 적용**:
- **모드 B**: 음을 높은음자리표에 1개 표시(+ 계이름 라벨 병기) → 반대 체계 이름 4지선다. "이 위치=솔=G" 3중 연결.
- **모드 C (`next` 유형)**: 지금까지 붙은 조표를 표준 위치로 오선에 표시(= 실제 조표 모양) → 다음 조표 4지선다. `nth` 유형은 배지 유지(위치 개념과 무관).
- **모드 D (`notes2name` 방향)**: 화음을 오선에 쌓아 표시(임시표 포함) → 코드 이름 4지선다. `name2notes` 방향은 심볼 카드 유지(보기 4개를 미니 오선으로 그리는 건 과함, YAGNI).
...
### 6.5 모드 D — 피아노 코드 (Chord)
**목표**: 삼화음·7화음의 **구성음 ↔ 코드 이름**을 양방향으로 익힌다.

**코드 범위(난이도)**: 쉬움 = 삼화음 4종(메이저 `maj`·마이너 `min`·증 `aug`·감 `dim`), 보통 = + 7화음 3종(도미넌트7 `7`·메이저7 `maj7`·마이너7 `m7`). **루트는 자연음 7개(C~B)만** 사용 → 스펠링이 ♯/♭ 1개 이내로 유지된다. **단 하나의 예외 — B 증3화음(Baug = B·D♯·F𝄪, 5음이 겹올림표)은 출제 풀에서 제외**한다(입문 범위 밖이고 `noteToFreq`는 홑임시표만 파싱). 나머지 자연루트×품질 조합은 전부 임시표 ±1 이내.

**정확한 스펠링(핵심)**: 화음 구성음은 **루트에서 3도씩 쌓는 글자**(루트, +2글자=3음, +4글자=5음, +6글자=7음)를 먼저 정하고, 반음 간격에 맞춰 ♯/♭을 붙인다. 예) **D 메이저 = D F♯ A** (G♭ 아님), C 감(dim) = C E♭ G♭. 즉 letterOffset `[0,2,4,(6)]` + 품질별 semitoneOffset로 계산하며, 각 음의 임시표 = `(루트반음 + semitoneOffset) − 그 글자의 자연 반음`.

| 품질 | 반음(루트 기준) | 예(C) | 심볼 |
|---|---|---|---|
| maj | 0·4·7 | C E G | C |
| min | 0·3·7 | C E♭ G | Cm |
| aug | 0·4·8 | C E G♯ | Caug |
| dim | 0·3·6 | C E♭ G♭ | Cdim |
| dom7 | 0·4·7·10 | C E G B♭ | C7 |
| maj7 | 0·4·7·11 | C E G B | Cmaj7 |
| min7 | 0·3·7·10 | C E♭ G B♭ | Cm7 |

**양방향 출제(둘 다)**: 문제마다 방향 랜덤.
- `notes2name`: 구성음(예 "도·미·솔") 카드 제시 → **코드 이름 4지선다**(C·Am·G·Fdim…).
- `name2notes`: 코드 이름(예 "Cm7") 카드 제시 → **구성음 세트 4지선다**.

**공통 처리(엔진 재사용)**:
- 보기 key는 **코드 id**(root+suffix, 예 `'Cm7'`), `labelFor(id)`가 방향에 따라 심볼 또는 구성음 이름을 렌더(언어/표기 축 따름). answer = 정답 코드 id.
- 오답 보기 = 서로 다른 코드 3개(중복 없이).
- 정답·"들어보기" 시 **`playChord()`로 화음 동시 재생**(audio.js 기존 함수 재사용). 힌트 = 품질 공식("메이저 = 루트+장3도+완전5도").
- 데이터: `data/chords.js`에 품질 상수 `CHORD_QUALITIES` + `makeChord(rootLetter, quality)` 팩토리(`makeNote`와 동일한 데이터-팩토리 패턴). **오선 화음/임시표 렌더는 MVP 생략**, 음이름 텍스트 카드로 표현(KISS).
